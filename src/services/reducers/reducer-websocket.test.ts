...
import { websocket, wsReducer } from './websocket';
  ...
  describe('Redux websocket store', () => {
    ...
    test('should handle WS_CONNECTION_ERROR', () => {
        ...
        expect(wsReducer(initialState, { type: WS_CONNECTION_ERROR })).toEqual(state)
    });
    ...
    test('should handle WS_GET_DATA', () => {
        ...
        expect(wsReducer(initialState, { type: WS_GET_DATA, payload: {success: true, orders: orders, totalToday: 0, total: 0} })).toEqual(state)
    });
  });